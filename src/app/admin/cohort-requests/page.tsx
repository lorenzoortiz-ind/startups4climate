...
import {
  loadCohortRequests,
  loadCohortRequestCounts,
  type RequestStatus,
  type CohortRequestRow,
  type CohortRequestCounts as CountsMap,
} from '@/lib/admin-data/cohort-requests'
...
export default function AdminCohortRequestsPage() {
  ...
  const refreshCounts = useCallback(async () => {
    try {
      setCounts(await loadCohortRequestCounts({ isDemo }))
    } catch (err) {
  ...
  const loadTab = useCallback(
    async (status: RequestStatus) => {
      ...
      setError(null)
      try {
        setRows(await loadCohortRequests({ isDemo }, status))
      } catch (err) {
