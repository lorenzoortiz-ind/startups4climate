  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
        ...
        {/* Results panel */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
          <MetricCard label="Customer Acquisition Cost" value={`$${calc.cac.toLocaleString('es')}`} sub="Costo de adquirir un cliente" color="#0D9488" />
          <MetricCard label="Margen Bruto" value={`$${calc.grossMargin.toLocaleString('es')}`} sub={`${calc.grossMarginPct}% sobre ingresos`} color={calc.grossMarginPct > 50 ? '#0D9488' : '#2A222B'} />
          ...
          <div style={{ background: 'var(--color-bg-card)', borderRadius: 12, border: '1px solid var(--color-border)', padding: '1rem', boxShadow: 'var(--shadow-card)' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
              <span style={{ fontFamily: 'var(--font-body)', fontSize: '1.75rem', fontWeight: 700, color: calc.ltvCacRatio >= 3 ? '#0D9488' : calc.ltvCacRatio >= 1 ? '#2A222B' : '#DC2626' }}>
                {calc.ltvCacRatio}x
              </span>
              {calc.ltvCacRatio >= 3 ? <TrendingUp size={16} color="#0D9488" /> : <TrendingDown size={16} color="#2A222B" />}
            </div>
            ...
            <div style={{ marginTop: '0.5rem', height: 4, borderRadius: 2, background: 'var(--color-bg-muted)' }}>
              <div style={{ height: '100%', borderRadius: 2, background: calc.ltvCacRatio >= 3 ? '#0D9488' : '#2A222B', width: `${Math.min(100, (calc.ltvCacRatio / 5) * 100)}%`, transition: 'width 0.6s' }} />
            </div>
          </div>
          <div style={{ background: 'var(--color-bg-card)', borderRadius: 12, border: '1px solid var(--color-border)', padding: '1rem', boxShadow: 'var(--shadow-card)' }}>
            ...
            <div style={{ fontFamily: 'var(--font-body)', fontSize: '1.75rem', fontWeight: 700, color: calc.greenPremium > 0 ? '#2A222B' : '#0D9488' }}>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', borderRadius: 8, border: `1px solid ${focused ? '#0D9488' : 'var(--color-border)'}`, background: 'var(--color-bg-primary)', overflow: 'hidden', transition: 'border 0.15s', boxShadow: focused ? '0 0 0 2px rgba(13,148,136,0.08)' : 'none' }}>
        {prefix && <span style={{ padding: '0 0.5rem', fontFamily: 'var(--font-body)', fontSize: '0.8125rem', color: 'var(--color-text-muted)', borderRight: '1px solid var(--color-border)', background: 'var(--color-bg-muted)' }}>{prefix}</span>}
        ...
        />
        {suffix && <span style={{ padding: '0 0.5rem', fontFamily: 'var(--font-body)', fontSize: '0.8125rem', color: 'var(--color-text-muted)', borderLeft: '1px solid var(--color-border)', background: 'var(--color-bg-muted)' }}>{suffix}</span>}
      </div>
    </div>
  )
}
